...
def test_build_runtime_configはmark_termsをmarkTermsへ伝播する():
    """[unit] build_runtime_config が riddle_mark_terms を camelCase キー markTerms へ伝播する。"""
    config = SimpleNamespace(
        riddle_trigger="both",
        riddle_open_delay_ms=150,
        riddle_close_delay_ms=100,
        ...
        riddle_image_popup=True,
        riddle_nested=True,
        riddle_mark_terms=False,
        riddle_table_align="left",
    )

    payload = build_runtime_config(config)

    assert payload["markTerms"] is False


@pytest.mark.sphinx("html", testroot="pages", warningiserror=True)
def test_実ビルドでriddle_config要素のmarkTermsが未指定時は既定Trueになる(app):
    """[integration] conf 未指定でビルドした #riddle-config JSON の markTerms が既定 True。"""
    app.build()
    html = (Path(app.outdir) / "index.html").read_text(encoding="utf-8")

    payload = _extract_riddle_config_json(html)
    assert payload["markTerms"] is True, (
        f"未指定時の既定 markTerms=True が入っていない（payload={payload!r}）"
    )
...
@pytest.mark.sphinx(
    ...
    testroot="pages",
    warningiserror=True,
    confoverrides={"riddle_mark_terms": False},
)
def test_実ビルドでriddle_config要素のmarkTermsがconf上書き値Falseを反映する(app):
    """[integration] 実ビルドで #riddle-config JSON に conf 上書き値
    （riddle_mark_terms=False）が markTerms として反映される（config.py の登録 →
    build_runtime_config の伝播 → inject_runtime_config の注入の配信経路の確証）。"""
    app.build()
    html = (Path(app.outdir) / "index.html").read_text(encoding="utf-8")

    payload = _extract_riddle_config_json(html)
    assert payload["markTerms"] is False, (
        "実ビルドの #riddle-config JSON に conf 上書き値 markTerms=False が反映されていない"
        f"（実際の payload={payload!r}）"
    )
...
def test_build_runtime_configはtable_alignをtableAlignへ伝播する():
    """[unit] build_runtime_config が riddle_table_align を camelCase キー tableAlign へ伝播する。"""
    config = SimpleNamespace(
        riddle_trigger="both",
        riddle_open_delay_ms=150,
        riddle_close_delay_ms=100,
        ...
        riddle_nested=True,
        riddle_mark_terms=True,
        riddle_table_align="center",
    )

    payload = build_runtime_config(config)

    assert payload["tableAlign"] == "center"
