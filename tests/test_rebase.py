...
def test_サブディレクトリからのアンカーはbaseと連結される():
    ...
    result = rebase_refuri(
        "#sec", home_docname="index", page_docname="sub/page", builder=_builder("../index.html")
    )
...
def test_外部URLはスキップされget_relative_uriを呼ばない():
    ...
    result = rebase_refuri(
        "https://example.com/x", home_docname="index", page_docname="index", builder=builder
    )
...
def test_falsyなrefuriはそのまま返る():
    ...
    assert rebase_refuri(None, home_docname="i", page_docname="i", builder=builder) is None
...
def test_クロスドキュメント相対はディレクトリと結合される():
    ...
    assert (
        rebase_refuri(
            "other.html#x", home_docname="index", page_docname="index", builder=_builder("index.html")
        )
        == "other.html#x"
    )
    ...
    assert (
        rebase_refuri(
            "other.html#x", home_docname="index", page_docname="p", builder=_builder("sub/index.html")
        )
        == "sub/other.html#x"
    )
...
def test_クロスドキュメント相対のフラグメントなしも扱える():
    ...
    assert (
        rebase_refuri(
            "other.html", home_docname="index", page_docname="p", builder=_builder("sub/index.html")
        )
        == "sub/other.html"
    )
...
def test_rebase_internal_referencesがsubtree内のrefuriを書き換える():
    ...
    rebase_internal_references(
        para, home_docname="index", page_docname="sub/p", builder=_builder("../index.html")
    )
