...
def test_term_titleを付加しない場合はriddle_term_titleが含まれない():
    ...
    result = render_definition(builder, object(), include_term_title=False, term_text="フー")
...
def test_term_titleが定義本文より前に付加される():
    ...
    result = render_definition(builder, object(), include_term_title=True, term_text="フー")
...
def test_term_textが空ならタイトルを付加しない():
    ...
    result = render_definition(builder, object(), include_term_title=True, term_text=None)
...
def test_無切り詰めで定義の全要素が残る(app):
    ...
    for needle in ("最初の段落。", "二番目の段落。", "項目一", "項目二", "項目三", "print"):
