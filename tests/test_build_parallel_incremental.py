...
def test_増分ビルドで定義側の画像差替え後に参照ページtemplateの宛先が追従し404にならない(
    app,
):
...
def test_並列ビルド後に増分で定義変更すると参照ページtemplate本文が最新へ置換される(
    app,
):
...
def test_増分ビルドでhome自身が参照ページの場合に自己注入templateが最新へ更新される(
    app,
):
    ...
    assert old_body in text, (
        "前提崩れ: deep/glossary.rst の selfword 定義本文が見つからない"
    )
...
def test_同一ページ参照ではnote_dependencyに自己依存を記録せずクロスページ参照は記録する(
    app,
):
...
def test_並列ビルドで実際にワーカープロセスがforkされた上で注入が成立する(
    ...
    monkeypatch.setattr(_builders_mod.ParallelTasks, "add_task", spy_add_task)
...
def test_並列増分ビルドで敵対的相対リンクが出力ルート内へクランプされ続ける(app):
    ...
    def _assert_clamped_within_root(href: str, phase: str) -> None:
        ...
        assert not (
            "://" in path_part or path_part.startswith(("//", "/", "data:", "mailto:"))
...
def test_増分ビルドでtermidが消えた定義変更時に参照ページの古いtemplateが残存しない(
    app,
):
...
def test_並列ビルドの注入templateが直列ビルドと完全一致する(
    ...
    assert serial_templates, "前提崩れ: 直列ビルドの出力に注入 template が 1 件も無い"
