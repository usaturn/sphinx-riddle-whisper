...
// M-1 回帰: mouseenter で予約された保留 open は、ポップ内 term click（新タブ遷移）で
// 取り消され、openDelay 経過後もレベル2が元タブで開かない。
test("ネスト: mouseenter 直後のポップ内 term click は保留 open を取り消す", () => {
  // Arrange: fake timer で install し、レベル1を開いてポップ内 term へ mouseenter
  const doc = docWithNestedTerms();
  const timers = installWithTimers(doc);
  const level1 = openLevel1(doc);
  const link = level1.querySelector('a[href="#term-b"]');
  link.dispatchEvent(
    new doc.defaultView.MouseEvent("mouseenter", { bubbles: false, cancelable: true }),
  );

  // Act: openDelay 未経過のまま click → その後 openDelay 経過
  const event = new doc.defaultView.MouseEvent("click", {
    bubbles: true,
    cancelable: true,
  });
  ...
  // Assert: tick 後もレベル2は開かず、既定動作（新タブ遷移）は生きている
  const nested = doc.querySelector(NESTED_SELECTOR);
  assert.equal(
    nested === null || nested.hasAttribute("hidden"),
    true,
    "click 後に openDelay が経過してもレベル2を開かない",
  );
  assert.equal(event.defaultPrevented, false, "既定動作（新タブ遷移）を殺さない");
});

// M-1 回帰（focusin 起点）: focus で予約された保留 open も click で取り消される。
// jsdom は FocusEvent の dispatch に難があるため plain Event で代替する（既存流儀）。
test("ネスト: focusin 直後のポップ内 term click は保留 open を取り消す", () => {
  // Arrange
  const doc = docWithNestedTerms();
  const timers = installWithTimers(doc);
  const level1 = openLevel1(doc);
  const link = level1.querySelector('a[href="#term-b"]');
  link.dispatchEvent(
    new doc.defaultView.Event("focusin", { bubbles: false, cancelable: true }),
  );
  ...
  // Assert
  const nested = doc.querySelector(NESTED_SELECTOR);
  assert.equal(nested === null || nested.hasAttribute("hidden"), true);
  assert.equal(event.defaultPrevented, false);
});

// M-1 回帰（openDelayMs: 0）: setTimeout(0) でも click 同期処理の後に発火するため、
// 保留 open の取り消しが必要（遅延 0 でも競合窓が存在する）。
test("ネスト: openDelayMs=0 でもポップ内 term click は保留 open を取り消す", () => {
  // Arrange
  const doc = docWithNestedTerms();
  const timers = installWithTimers(doc, { openDelayMs: 0 });
  const level1 = openLevel1(doc);
  const link = level1.querySelector('a[href="#term-b"]');
  link.dispatchEvent(
    new doc.defaultView.MouseEvent("mouseenter", { bubbles: false, cancelable: true }),
