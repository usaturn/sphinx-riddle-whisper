...
/** fake timer 注入で install し、click で両レベルを開いて {timers, level1, nested} を返す。 */
function openBothWithTimers(doc) {
...
test("hover: レベル1からレベル2ポップへの移動では閉じない", () => {
  // Arrange
  const doc = docWithNestedTerms();
  const { timers, level1, nested } = openBothWithTimers(doc);
  const { MouseEvent } = doc.defaultView;
...
test("結合: レベル2ポップ内のリンクへ target=_blank と rel が付与される", () => {
  ...
  const nestedLink = doc
    .querySelector(NESTED_SELECTOR)
    .querySelector('a[href="#term-c"]');
