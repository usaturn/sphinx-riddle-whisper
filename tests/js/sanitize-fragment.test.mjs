...
import { sanitizeFragment } from "../../src/sphinx_riddle_whisper/static/riddle.js";
...
test("二次防御走査: 危険要素を孕む混在 fragment でも例外なく走査し良性物を保持する", () => {
  ...
  const jsHref = [...frag.querySelectorAll("a")].filter((el) => {
    const v = (el.getAttribute("href") || "").trim().toLowerCase();
    return v.startsWith(dangerScheme);
  });
  assert.equal(jsHref.length, 0, "走査後も危険スキーム href が残っている");
