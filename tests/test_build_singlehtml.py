...
def test_singlehtmlビルドがwarning_as_errorで警告ゼロで完走し単一index_htmlのみ生成する(
    app,
):
...
@pytest.mark.sphinx("singlehtml", testroot="pages", warningiserror=True)
def test_term参照hrefが同一ページ内フラグメントで別ページ相対パスに再ベースされない(
    app,
):
    ...
    assert term_hrefs, "本文に term-0 を指す :term: 参照リンクが無い（前提崩れ）"
...
@pytest.mark.sphinx("singlehtml", testroot="pages", warningiserror=True)
def test_注入template内のref同一home内アンカーがsinglehtml同一ページfragmentへ解決される(
    app,
):
...
@pytest.mark.sphinx("singlehtml", testroot="pages", warningiserror=True)
def test_注入template内のdocクロスドキュメントリンクがsinglehtml同一ページfragmentへ解決される(
    app,
):
    ...
    assert 'href="../topic.html"' not in template, (
        "html/dirhtml 形 ../topic.html が残存（singlehtml 単一ページ上で 404 になる）"
    )
    assert 'href="topic.html"' not in template, (
...
@pytest.mark.sphinx("singlehtml", testroot="pages", warningiserror=True)
def test_注入template内の相対画像がsinglehtml集約後も_imagesへ委譲され404にならない(
    app,
):
...
@pytest.mark.sphinx("singlehtml", testroot="pages", warningiserror=True)
def test_singlehtml注入template内の敵対相対リンクが出力ルート外へ脱出再ベースされない(
    app,
):
...
@pytest.mark.sphinx("singlehtml", testroot="pages", warningiserror=True)
def test_home自身の同一home内term参照refid形でもtemplate注入され二重フラグメント化しない(
    app,
):
...
def test_定義subtree内アンカーへのref_refidがsinglehtml集約後も二重フラグメント化せず素のフラグメントで残る(
    ...
    inner_hrefs = re.findall(r'href="([^"]*selfanchor-inner[^"]*)"', template)
    assert inner_hrefs, (
...
@pytest.mark.sphinx("singlehtml", testroot="pages", warningiserror=True)
def test_定義内の外部URLリンクがsinglehtml集約後もverbatim保持されdocument連結されない(
    app,
):
    ...
    misconcatenated = [h for h in hrefs if "example.com/baz" in h and h != external_url]
...
@pytest.mark.sphinx("singlehtml", testroot="pages", warningiserror=True)
def test_定義本文同士が相互参照するterm参照もsinglehtml集約後に同一ページfragmentへ閉じる(
    app,
):
