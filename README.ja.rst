...
- **増分ビルド対応** — 用語の定義元が変更されると、その用語を参照しているページが
  再ビルドされます。
- **ネストポップアップ** — ポップアップ内の ``:term:`` リンクへのホバー／
  フォーカスで2段目のポップアップを開ける（固定2段・設定で無効化可能）。
  クリックした場合は用語集を新しいタブで開きます。
- **リンクの新タブ表示** — ポップアップ内のリンクは ``rel="noopener noreferrer"``
  付きで新しいブラウザタブで開きます（画像ライトボックス有効時の画像リンクは
  ライトボックス動作のままです）。
...
リンクにホバーまたはフォーカスすると、2段目のポップアップが1段目に重ねて
表示され、他の用語を参照する定義をページを離れずに辿れます。クリックした
場合は、その用語の定義元（用語集）が新しいタブで開きます。ネストは固定2段で、
2段目のポップアップ内の ``:term:`` は通常のリンクとして機能します。1段目に表示中の
用語と同じ用語へのリンクでは重複表示しません。Esc キーは内側（2段目）から順に
閉じます。``False`` にするとネストポップアップは開かず、ポップアップ内の
``:term:`` リンクは他のポップアップ内リンクと同様、用語集を新しいタブで
開くだけになります。``riddle_trigger = "click"``\ ではホバー／フォーカスの
経路が無いためネストポップアップは開けず、ポップアップ内の ``:term:``
リンクは用語集を新しいタブで開くだけになります。
...
マーキングは付きません（リンク色はテーマのまま変更されません）。なお、ポップアップ内に表示される ``:term:`` リンクにはマーキングは付きません。
